$(document).ready(function() {
	...
	$('#movie_title_search_term').on('click', function(event) {
		...
		var movie_title = $(this).siblings('.search_term').val();
		if (!movie_title) {
	...
	$('#movie_actor_search_term').on('click', function(event) {
		event.preventDefault();
		var person = $(this).siblings('.search_term').val();
		if (!person) {
...
var showPosters = function(movie) {
	...
	if (movie.poster_path !== null) {
		...
		image.attr('src', 'images/noposter.jpg');
...
var displayPersons = function(persons) {
	if (persons.hasOwnProperty('results')) {
		$('.movie_details').hide();
		$('.person').show();
		$('#results').append('<div class="people"></div>');
		$.each(persons.results, function(i, person) {
			var person = createMovieCastDetail(person);
...
// Call the TMDB API and get person information
var getPersonDetail = function(person_id) {
	// the parameters we need to pass in our request to TMDB's API
	var request = {
		api_key: tmdb_api_key
	};
...
// Call the TMDB API and get persons from search
var getPersons = function(search_term) {
	...
	}).fail(function(jqXHR, error, errorTrown) {
		$('#error').append(error).show();
	});
};